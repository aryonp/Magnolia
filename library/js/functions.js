function confirmBox(theLink,type,addMsg,iID) {
	if (type == 'del') {
		var confirmMsg = 'Do you really want to delete access for\n';
		var paramQ = 'did';
	}
	else if (type == 'sudo') {
		 var confirmMsg  = 'Switch user to';
		 var paramQ = 'sid';
	}
	if (confirmMsg == '' || typeof(window.opera) != 'undefined') {
		return true;
	}
	var is_confirmed = confirm(confirmMsg + ' ' + addMsg );
	if (is_confirmed) {
		if (typeof(theLink.href) != 'undefined') {
			theLink.href += '&' + paramQ + '=' + iID;
		} 
		else if (typeof(theLink.form) != 'undefined') {
			theLink.form.action += '&' + paramQ + '=' + iID;
		}
	} 
	return is_confirmed;
}
...
function logout() {
	var logTxt = confirm("Are you sure you want to log out from the system?");
	if (logTxt == true) {
		location.href = "/storix/logout.php";
	}
}
...
function NewCal(N,I,L,S,K,H){
	...
	if(I!=null){
		Cal.Format=I.toUpperCase()	
	}
	if(K!=null){
		if(K.toUpperCase()=="ARROW"){
			Cal.Scroller="ARROW"
		}
		else{
	...
	exDateTime=document.getElementById(N).value;
	if(exDateTime!=""){
		var R;
		var Q;
		var G;
		...
		var D;
		var B;
		R=exDateTime.indexOf(DateSeparator,0);
		Q=exDateTime.indexOf(DateSeparator,(parseInt(R)+1));
		var E=parseInt(Cal.Format.toUpperCase().lastIndexOf("M"))-parseInt(Cal.Format.toUpperCase().indexOf("M"))-1;
		
		if((Cal.Format.toUpperCase()=="DDMMYYYY")||(Cal.Format.toUpperCase()=="DDMMMYYYY")){
			if(DateSeparator==""){
				C=exDateTime.substring(2,4+E);
				F=exDateTime.substring(0,2);
				O=exDateTime.substring(4+E,8+E)
			}
			else{
				C=exDateTime.substring(R+1,Q);
				F=exDateTime.substring(0,R);
				O=exDateTime.substring(Q+1,Q+5)
			}
		}
		else{
			if((Cal.Format.toUpperCase()=="MMDDYYYY")||(Cal.Format.toUpperCase()=="MMMDDYYYY")){
				if(DateSeparator==""){
					C=exDateTime.substring(0,2+E);
					F=exDateTime.substring(2+E,4+E);
					O=exDateTime.substring(4+E,8+E)
				}
				else{
					C=exDateTime.substring(0,R);
					F=exDateTime.substring(R+1,Q);
					O=exDateTime.substring(Q+1,Q+5)
				}
			}
			else{
				if((Cal.Format.toUpperCase()=="YYYYMMDD")||(Cal.Format.toUpperCase()=="YYYYMMMDD")){
					if(DateSeparator==""){
						C=exDateTime.substring(4,6+E);
						F=exDateTime.substring(6+E,8+E);
						O=exDateTime.substring(0,4)
					}
					else{
						C=exDateTime.substring(R+1,Q);
						F=exDateTime.substring(Q+1,Q+3);
						O=exDateTime.substring(0,R)
					}
				}
			}
		}
		if(isNaN(C)){
			M=Cal.GetMonthIndex(C)
		}
		else{
			M=parseInt(C,10)-1
		}
		...
		if((parseInt(F,10)<=Cal.GetMonDays())&&(parseInt(F,10)>=1)){
			Cal.Date=F}A=/^\d{4}$/;
			if(A.test(O)){
				Cal.Year=parseInt(O,10)
			}
			if(Cal.ShowTime==true){
				...
				J=exDateTime.substring(G,(G)-2);
				Cal.SetHour(J);
				P=exDateTime.substring(G+1,G+3);
				Cal.SetMinute(P);
				D=exDateTime.substring(tSp2+1,tSp2+3);
				Cal.SetSecond(D);
				window.status=J+":"+P+":"+D
			}	
	}
...
function RenderCal(){
	...
	winCal.document.writeln("<script>var winMain=window.opener;<\/script>");
	...
	if(Cal.Scroller=="DROPDOWN"){
		G+='<td align=\'left\'><select name="MonthSelector" onChange="javascript:winMain.Cal.SwitchMth(this.selectedIndex);winMain.RenderCal();">\n';
		for(J=0;J<12;J++){
			if(J==Cal.Month){
				F="Selected"	
			}
			...
			G+="<option "+F+" value >"+MonthName[J]+"\n"}G+="</select></td>";
			G+='\n<td align=\'right\'><a href="javascript:winMain.Cal.DecYear();winMain.RenderCal()"><b><font color="'+YrSelColor+'"><</font></b></a><font face="Verdana" color="'+YrSelColor+'" size=2><b> '+Cal.Year+' </b></font><a href="javascript:winMain.Cal.IncYear();winMain.RenderCal()"><b><font color="'+YrSelColor+'">></font></b></a></td></tr></table></td>\n';
			G+="</tr>"		
	}
	else{
		if(Cal.Scroller=="ARROW"){
			G+="<td align='center'><a href='javascript:winMain.Cal.DecYear();winMain.RenderCal();'>- </a></td>";
			G+="<td align='center'><a href='javascript:winMain.Cal.DecMonth();winMain.RenderCal();'>&lt;</a></td>";
			G+="<td align='center' width='70%'><font face='Verdana' size='2' color='"+YrSelColor+"'><b>"+Cal.GetMonthName(ShowLongMonth)+" "+Cal.Year+"</b></font></td>";
			G+="<td align='center'><a href='javascript:winMain.Cal.IncMonth();winMain.RenderCal();'>&gt;</a></td>";
			G+="<td align='center'><a href='javascript:winMain.Cal.IncYear();winMain.RenderCal();'>+</a></td>";
			G+="</tr></table></td></tr>"		
		}
	}
	if((ShowMonthYear)&&(Cal.Scroller=="DROPDOWN")){
		G+="<tr><td colspan='7'><font face='Verdana' size='2' align='center' color='"+MonthYearColor+"'><b>"+Cal.GetMonthName(ShowLongMonth)+" "+Cal.Year+"</b></font></td></tr>\n"
	}
	...
	for(J=0;J<7;J++){
		G+="<td align='center'><font face='Verdana' size='2'>"+K[J].substr(0,WeekChar)+"</font></td>"}G+="</tr>";
		winCal.document.write(G);
		...
		for(I=1;I<=Cal.GetMonDays();I++){
			...
			if((I==dtToday.getDate())&&(Cal.Month==dtToday.getMonth())&&(Cal.Year==dtToday.getFullYear())){
				H=GenCell(I,true,TodayColor)
			}
			else{
				if((I==selDate.getDate())&&(Cal.Month==selDate.getMonth())&&(Cal.Year==selDate.getFullYear())){
					H=GenCell(I,true,SelDateColor)
				}
				else{
					if(MondayFirstDay==true){
						if(L%7==0){
							H=GenCell(I,false,SundayColor)	
						}
						else{
							if((L+1)%7==0){
								H=GenCell(I,false,SaturdayColor)	
							}
							else{
								H=GenCell(I,null,WeekDayColor)	
							}
						}
					}
					else{
			...
			D=D+H;
			if((L%7==0)&&(I<Cal.GetMonDays())){
				D=D+"</tr>\n<tr>"}}winCal.document.writeln(D);
				if(Cal.ShowTime){
					var C;
					C=Cal.getShowHour();
					B="<tr>\n<td colspan='7' align='center'>";
					B+="<input type='text' name='hour' maxlength=2 size=1 style=\"WIDTH: 22px\" value="+C+' onchange="javascript:winMain.Cal.SetHour(this.value)">';B+=" : ";
					B+="<input type='text' name='minute' maxlength=2 size=1 style=\"WIDTH: 22px\" value="+Cal.Minutes+' onchange="javascript:winMain.Cal.SetMinute(this.value)">';
					if(Cal.ShowSeconds){
						B+=" : ";
						B+="<input type='text' name='second' maxlength=2 size=1 style=\"WIDTH: 22px\" value="+Cal.Seconds+' onchange="javascript:winMain.Cal.SetSecond(this.value)">'
					}
					if(TimeMode==12){
						var E=(Cal.AMorPM=="AM")?"Selected":"";
						var A=(Cal.AMorPM=="PM")?"Selected":"";
					...
					B+="\n</td>\n</tr>";
					winCal.document.write(B)}winCal.document.writeln("\n</table>");
...
function GenCell(H,D,F){
	...
	if(Cal.ShowTime){
		E="winMain.document.getElementById('"+Cal.Ctrl+"').value+=' '+winMain.Cal.getShowHour()+':'+winMain.Cal.Minutes";
		if(Cal.ShowSeconds){
			E+="+':'+winMain.Cal.Seconds"	
		}
		if(TimeMode==12){
	...
	B="<td "+C+" width="+CellWidth+" align='center'><font face='verdana' size='2'"+G+"<a href=\"javascript:winMain.document.getElementById('"+Cal.Ctrl+"').value='"+Cal.FormatDate(A)+"';"+E+';window.close();">'+A+"</a>"+vHLstr2+"</font></td>";
	return B
}
...
function SetHour(A){
	...
	if(E.test(A)&&(parseInt(A,10)<=C)&&(parseInt(A,10)>=B)){
...
function FormatDate(B){
	...
	if(this.Format.toUpperCase()=="DDMMYYYY"){
		...
		if(this.Format.toUpperCase()=="DDMMMYYYY"){
			return(B+DateSeparator+this.GetMonthName(false)+DateSeparator+this.Year)	
		}
		else{
			if(this.Format.toUpperCase()=="MMDDYYYY"){
				return(A+DateSeparator+B+DateSeparator+this.Year)
			}
			else{
				if(this.Format.toUpperCase()=="MMMDDYYYY"){
					return(this.GetMonthName(false)+DateSeparator+B+DateSeparator+this.Year)		
				}
				else{
					if(this.Format.toUpperCase()=="YYYYMMDD"){
						return(this.Year+DateSeparator+A+DateSeparator+B)
					}
					else{
						if(this.Format.toUpperCase()=="YYYYMMMDD"){
							return(this.Year+DateSeparator+this.GetMonthName(false)+DateSeparator+B)
						}
						else{
							return(B+DateSeparator+(this.Month+1)+DateSeparator+this.Year)
						}
					}
				}
			}
...
try {var pageTracker = _gat._getTracker("UA-7331780-3");pageTracker._trackPageview();} catch(err) {}